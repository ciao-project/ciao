...
func (d workloadStorage) Populate() error {
	...
	for _, line := range lines {
		...
		err = d.ds.create(d.name, workloadID, volumeID, bootable, persistent, size, sourceType, sourceID, tag)
		if err != nil {
...
func (ds *sqliteDB) createWorkloadStorage(tx *sql.Tx, workloadID string, storage types.StorageResource) error {
	_, err := tx.Exec("INSERT INTO workload_storage (workload_id, volume_id, bootable, persistent, size, source_type, source_id, tag) VALUES (?, ?, ?, ?, ?, ?, ?, ?)", workloadID, storage.ID, storage.Bootable, storage.Persistent, storage.Size, string(storage.SourceType), storage.SourceID, storage.Tag)
...
func (ds *sqliteDB) getWorkloadStorage(ID string) (*types.StorageResource, error) {
	...
	err := row.Scan(&r.ID, &r.Bootable, &r.Persistent, &r.Size, &sourceType, &r.SourceID, &r.Tag)
