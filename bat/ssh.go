...
//
//session should be created successfully and wait for commands
func createSSHClient(ctx context.Context, c SSHClient) (*ssh.Client, error) {
	...
	ClientFullAddress := net.JoinHostPort(c.ClientIP, c.ClientPort)
	...
	return dialSSH(ctx, "tcp", ClientFullAddress, config)
}
